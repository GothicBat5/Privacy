import { addressesThunk, initEvent,
    organizationThunk, retentionPoliciesThunk,
    serverEvent, startLogoutListener,
    userSettingsThunk, userThunk,
    welcomeFlagsActions,
} from '@proton/account';
...
import { canLoadRunner, shouldLoadMigrationWorker,} from './helpers/encryptedSearch/migration-system/helpers/shouldLoadMigrationWorker';
...
export const bootstrapApp = async ({ config, signal }: { config: ProtonConfig; signal?: AbortSignal }) => {
    ...
    if (isElectronMail) 
    {
        void import('@proton/shared/lib/desktop/bootstrapMailInboxDesktop').then((module) => {
            module.bootstrapMailInboxDesktop({config, authentication, api, });
        });
    }
    ...
    const run = async () => {
            ...
            .then(() => {
                ...
                if (unleashClient.isEnabled(CommonFeatureFlag.WebApiRateLimiter)) 
                {
                    ...
                    if (config) 
                    {
                        ...
                        api.apiRateLimiter.configure({ maxRequests: configParsed.maxRequests, windowMs: configParsed.windowMs,});
                    }
        ...
        const loadUser = async () => {
            ...
            const [scopes] = await Promise.all([bootstrap.enableTelemetryBasedOnUserSettings({ userSettings }),
        ...
        const loadPreload = () => {
            return Promise.all([dispatch(categoriesThunk()), dispatch(mailSettingsThunk()),
                dispatch(organizationThunk()), dispatch(addressesThunk()),
        ...
        const [MainContainer, userData] = await Promise.all([appContainerPromise,
            userPromise, bootstrap.loadCrypto({ appName, unleashClient }),
            unleashPromise,
        ]);
        ...
        if (!isEncryptedSearchMigrationSystemDisabled) 
        {
            ...
            {
                void migrationToolWorker({user: userData.user,
        ...
        return { ...userData, eventManager,
            unleashClient, history,
            store, MainContainer, };};
